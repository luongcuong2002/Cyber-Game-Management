...
public class BillDialog extends JDialog {
    ...
    public BillDialog(ComputerClient parentView, Computer computer) {
        ...
        int timeFeeTransferred = 0;
        for (int i = 0; i < computer.getListTransactionsTransfer().size(); i++) {
            timeFeeTransferred += computer.getListTransactionsTransfer().get(i).getTimeFee();
        }
        ...
        int serviceFeeTransferred = 0;
        for (int i = 0; i < computer.getListTransactionsTransfer().size(); i++) {
            serviceFeeTransferred += computer.getListTransactionsTransfer().get(i).getServiceFee();
        }
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (timeFee > 0) {
                    Data.listTransactionHistoryItems.add(new TransactionHistoryItem(
                            ...
                            computer.getTimeStart(),
                            Calendar.getInstance().getTime(),
                            timeFee,
                            0,
                            computer.getUserUsing().getUserGroupName() + " " + computer.getUserUsing().getUserName()
                            + " đã trả " + NumberFormat.getNumberInstance().format(timeFee) + " đồng" + " thời gian phí."
                    ));
                }

                if (computer.getServiceFee() > 0) {
                    Data.listTransactionHistoryItems.add(new TransactionHistoryItem(
                            computer.getUserUsing().getUserName(),
                            null,
                            Calendar.getInstance().getTime(),
                            ...
                            0,
                            computer.getUserUsing().getUserGroupName() + " " + computer.getUserUsing().getUserName()
                            + " đã trả " + NumberFormat.getNumberInstance().format(computer.getServiceFee()) + " đồng" + " phí dịch vụ bằng tiền mặt."
                    ));
                }

                if (computer.getListTransactionsTransfer().size() > 0) {
                    if (computer.getTimeFeeTransferred() > 0) {
                        Data.listTransactionHistoryItems.add(new TransactionHistoryItem(
                                computer.getUserUsing().getUserName(),
                                computer.getTimeStart(),
                                Calendar.getInstance().getTime(),
                                computer.getTimeFeeTransferred(),
                                0,
                                computer.getTimeFeeDescriptionForTransferTransaction()
                        ));
                    }

                    if (computer.getServiceFeeTransferred()> 0) {
                        Data.listTransactionHistoryItems.add(new TransactionHistoryItem(
                                computer.getUserUsing().getUserName(),
                                null,
                                Calendar.getInstance().getTime(),
                                computer.getServiceFeeTransferred(),
                                0,
                                computer.getServiceFeeDescriptionForTransferTransaction()
                        ));
                    }
                }
        ...
        tranferButton.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                new TransferDialog(BillDialog.this, computer).setVisible(true);
            }
        });
